...
export class ParentRepository {
  ...
  async create(
    data: Omit<
      Parent,
      'id' | 'createdAt' | 'updatedAt' | 'deletedAt' | 'students'
    >,
  ): Promise<Parent> {
    const persistenceModel = ParentMapper.toPersistence({
      ...data,
  ...
  async update(
    id: Parent['id'],
    payload: Partial<
      Omit<Parent, 'id' | 'password' | 'createdAt' | 'updatedAt' | 'deletedAt'>
    >,
  ): Promise<Parent> {
    const entity = await this.parentRepository.findOne({
      where: { id },
  ...
  async removeChild(studentId: Student['id'], parentId: Parent['id']) {
    ...
    parentEntity.students = parentEntity.students.filter(
      (item) => item.id.toString() !== studentId.toString(),
    );
    await this.parentRepository.save(parentEntity);
    return parentEntity;
  }
